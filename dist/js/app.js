var bmoreShootings = {
    init: function() {
        bmoreShootings.otherMobileFix();
        bmoreShootings.createMap();
        bmoreShootings.share();
    },
    share: function() {
        $(".icon-twitter").on("click", function() {
            ...
            var tweet = "Since 2013, there have been " + stats[0] + " shootings and " + stats[1] + " homicides " + area;
            var url = "http://data.baltimoresun.com/jin/homicides-near-you";
        ...
        $(".icon-facebook").on("click", function() {
            ...
            var picture = "http://data.baltimoresun.com/jin/homicides-near-you/images/thumb.png";
            var title = "How violent is your neighborhood?";
            var description = "Since 2013, there have been " + stats[0] + " shootings and " + stats[1] + " homicides " + area;
            var url = "http://data.baltimoresun.com/jin/homicides-near-you";
        ...
        window.onload = function() {
            ...
            var layerSource = {
                ...
                sublayers: [ {
                    ...
                    cartocss: "#bmore_shootings{marker-fill-opacity: 1;" + "marker-line-color: #daa520;" + "marker-line-width: 1;" + "marker-line-opacity: 1;" + "marker-placement: point;" + "marker-type: ellipse;" + "marker-width: 2;" + "marker-fill: #fff;" + "marker-allow-overlap: true;" + "[zoom>13]{marker-width: 6;}}"
                }, {
                    sql: "SELECT * FROM bmore_homicides",
                    cartocss: "#bmore_homicides{marker-fill-opacity: 1;" + "marker-line-color: #900020;" + "marker-line-width: 1;" + "marker-line-opacity: 1;" + "marker-placement: point;" + "marker-type: ellipse;" + "marker-width: 2;" + "marker-fill: #fff;" + "marker-allow-overlap: true;" + "[zoom>13]{marker-width: 6;}}"
                } ]
            };
            mainlayers = [];
            cartodb.createLayer(homicideMap, layerSource, {
                ...
                var homicidesTooltip = layer.leafletMap.viz.addOverlay({
                    type: "infobox",
                    layer: mainlayers[1],
                    template: '<div class="homicidesInfobox">' + "<p>{{location}}</p>" + "<p>KILLED: {{crimedate}}</p>" + "<p>WEAPON: {{weapon}}</p></div>",
                    width: 208,
                    fields: [ {
                        location: "location",
                        crimedate: "crimedate",
                ...
                var shootingsTooltip = layer.leafletMap.viz.addOverlay({
                    type: "infobox",
                    layer: mainlayers[0],
                    template: '<div class="shootingsInfobox">' + "<p>{{location}}</p>" + "<p>SHOT: {{crimedate}}</p></div>",
                    width: 208,
                    fields: [ {
                        location: "location",
                        crimedate: "crimedate"
            ...
            function createFullMap() {
                clearMap();
                $(".numbersIntro").text("Within Baltimore:");
                countPoints("SELECT * FROM bmore_shootings", "SELECT * FROM bmore_homicides");
                var lat = 39.289156;
                var lon = -76.58342;
                var zoomLvl = 12;
                if (isMobile) {
            ...
            var osmGeocoder = new L.Control.OSMGeocoder({
                ...
                position: "topright",
                text: "Locate",
                bounds: L.latLngBounds(L.latLng(39.194277, -76.724858), L.latLng(39.380019, -76.517491)),
                callback: function(results) {
                    if (results[0] != undefined) {
                        ...
                        alert("Error: This address is not in Baltimore");
                    }
                }
            });
